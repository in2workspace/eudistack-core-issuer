...
- **EUD-215 — `/oid4vci/v1/authorize` let a loose outer `state` query parameter override the `state` already committed to a pushed authorization request**: `AuthorizationServiceImpl.pushAuthorizationRequestAuthorization` built the redirect with `state != null ? state : parRequest.state()` — prioritizing whatever `state` accompanied the `/authorize` call over the one bound to the PAR (`parRequest.state()`). RFC 9126 §4 requires the opposite: once `request_uri` references a pushed request, the authorization server SHOULD ignore any other parameter on that call — only what was pushed is authoritative. Caught by two OIDF conformance suite tests: `fapi2-security-profile-final-ensure-different-state-inside-and-outside-request-object` (the suite deliberately sent a different, "incorrect" `state` alongside `request_uri` and got that one back instead of the PAR-bound value) and `fapi2-security-profile-final-state-only-outside-request-object-not-used` (the PAR carried no `state` at all, but one appeared in the response anyway because the outer param was accepted). Fixed by always using `parRequest.state()` for the PAR-redeemed redirect, dropping the outer `state` parameter from `pushAuthorizationRequestAuthorization` entirely (the direct, non-PAR path in `processDirectAuthorization` is unaffected — it has no pushed request to defer to).
