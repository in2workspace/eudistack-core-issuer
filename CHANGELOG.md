...
- **EUD-215 — the Token Status List (`draft-ietf-oauth-status-list`) `lst` claim used the wrong compression, undecodable by any conformant reader**: `TokenStatusListCredentialFactory.buildUnsigned` reused `encodedList` (the shared bitstring storage format — multibase + base64url + **GZIP**, RFC 1952) as-is for the `lst` claim, but the spec mandates plain **DEFLATE** (RFC 1951, no zlib/gzip wrapper). Caught by the OIDF conformance suite's `oid4vci-1_0-issuer-happy-flow` test (`OTSL-6.2 CheckCredentialStatus`: `DataFormatException: incorrect header check`). Now decodes back to the raw bitstring (`BitstringEncoder`) and re-compresses with raw DEFLATE (`Deflater(..., nowrap=true)`) before base64url-encoding for `lst` — the W3C Bitstring Status List JWT (which does use GZIP) is unaffected, it still reads `encodedList` directly.
