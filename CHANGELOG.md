...
- **EUD-215 — `JadesHeaderBuilderServiceImpl` published the self-signed root certificate in `x5c` whenever the QTSP's `credentials/info` chain response carried one**: HAIP §6.1 forbids a conformant issuer from including the root — the relying party is expected to already trust it out-of-band. `certInfo.certificates()` went straight into the header with no filtering. `buildHeader` now trims the trailing certificate when it's self-signed, before publishing `x5c` — applies to both credentials and status list tokens, since they share this one header-building code path (companion fix in `eudistack-core-verifier`, which stopped requiring a self-signed root to accept the chain). See `docs/EUD-178-compliance-eudi-certificacion/EUD-215/analisis-x5c-certificado-qtsp.md` (`eudistack-platform-dev`) for why this alone doesn't close `HAIP-6.1`/`HAIP-6.1.1` against sandbox STG today.
