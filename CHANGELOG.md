...
- **OID4VCI — nonce consumed before signature validation**: `ProofValidationServiceImpl.verifyProof()` deleted the nonce from the cache before verifying the JWT signature. A 502 mid-flight (e.g. network glitch or upstream timeout) caused the nonce to be lost, making every subsequent retry return 400 `invalid_proof: nonce is invalid or expired`. Refactored into a two-step flow: `checkNonce()` verifies the nonce exists but does NOT delete it; deletion only happens after `validateSignatureAccordingToHeader()` succeeds. Tests updated to assert the nonce is not consumed on signature failure.
- **OID4VCI — `CacheStore.get()` emits error on cache miss**: `CacheStore.get()` returned `Mono.error(NoSuchElementException)` when the key was absent. Callers using `.switchIfEmpty()` never saw the empty signal — `switchIfEmpty` only fires on `Mono.empty()`, not on `Mono.error()` — so any cache miss propagated as an uncaught exception. Fixed to return `Mono.empty()` on miss (null value from Guava `getIfPresent()`).
