...
- **EUD-169 — `GET /admin/v1/credential-catalog` opened to the tenant's operator (`LEAR`) role**: the operator, who is the actor that ultimately issues a credential, can now read the catalog (eligible delivery modes and schema ceiling per type) to know its options before attempting an issuance, instead of discovering them only against an issuance rejection. `PUT` remains administrator-only — the write authorization check no longer delegates to the read check, so this change cannot silently propagate to the write path.
...
- Added a second credential profile fixture (`gx.labelcredential.w3c.2`, unbound) to the test classpath so `CredentialCatalogTransactionalIT` can build a genuinely mixed multi-type payload against a real Postgres: concurrent writes now prove the type neither one declared survives untouched (ES-04), previously evidenced only indirectly (a partial single-type real-DB test). No production behavior change.

### Changed (EUD-169 — re-verification `/code-review`, H1)

- **`GET`/`PUT /admin/v1/credential-catalog` renamed to `.../api/v1/backoffice/credential-catalog`.** Pure path rename, no authorization or response-shape change (`EndpointsConstants.CREDENTIAL_CATALOG_PATH`): re-verification (H1) found the `/admin` prefix misleading now that AD-16 opened `GET` to the tenant's operator (`LEAR`) role as well as tenant-admin/SysAdmin — the path implied an admin-only surface the endpoint had already stopped being. Reverts to this application's own pre-existing convention for this functional area: the retired `DeliveryConfigController` (superseded by this Story, see the `Removed` entry above) used `/api/v1/backoffice/delivery-config/{credentialConfigurationId}` for an even more restricted (fully admin-only) predecessor. No change to authorization or response shape — still authenticated-only, `GET` still returns the full registry (enabled and disabled ids, needed by the settings UI to render both states), `PUT` still tenant-admin/SysAdmin-only. **Correction (re-verification `/code-review`, Ronda 3, 2026-09-10, `[F1]`):** the rename does widen this endpoint's CORS surface — `CorsConfig` registers `/api/**` with `allowedOriginPatterns=[*]` (`allowedMethods=[GET, POST, OPTIONS]`, no credentials), while `/admin/**` matched no CORS registration at all, so a cross-origin browser request carrying `Authorization` was previously blocked at preflight and now succeeds. Explicitly accepted as-is: this is the same CORS treatment already given to other `/api/**` endpoints (e.g. `GET /api/v1/me`), not a bespoke exposure introduced for this one; `PUT` stays outside `allowedMethods` so the write path is unaffected; and exploiting the read requires already holding a valid bearer token, at which point calling the API directly needs no browser at all. No code change.
...
### Fixed (EUD-169 — re-verification `/code-review`, M3/M4/N1/L2)

- **`TenantMismatchException` now goes through `handleSafe`, not `handleWith`** (N1) — the exception message embeds the caller-supplied `tokenTenant`/resolved-tenant values verbatim, the same class of gap already closed for the sibling handlers under F5.
- **A legacy env-suffixed tenant claim (e.g. `sandbox-stg`) no longer fails the tenant-match comparison on formatting alone** (M3) — `AccessTokenServiceImpl.getTokenTenant`/`PolicyContextFactory.extractTokenTenant` now apply `TenantIdentifiers.stripEnvSuffix` to the token side, mirroring the normalization `TenantDomainWebFilter` already applies to the resolved-tenant side.
...
- **Tech Debt (EUDISTACK-650) — Decouple `Issuance` persistence from the domain (H-05)**: `Issuance` no longer carries `@Table`/`@Column`/`@Version`/auditing annotations, and `IssuanceServiceImpl` no longer injects `R2dbcEntityTemplate` directly — same pattern already used by the `apiclient` bounded context (port + infrastructure entity + mapper). Added `IssuancePort` (domain), `IssuanceEntity` + `IssuanceMapper` + `IssuanceR2dbcRepository` + `IssuanceR2dbcAdapter` (infrastructure); `CredentialExpirationScheduler` and the Testcontainers ITs that seed `issuance` rows directly now depend on `IssuancePort` instead of the old Spring Data repository. Added an ArchUnit rule (`domainShouldNotDependOnSpringData`) blocking `..domain..` → `org.springframework.data..` going forward, with a documented, narrow exemption for the pre-existing `Tenant*` entities and `TenantCredentialProfileServiceImpl` (same debt, out of this ticket's scope). No behavior change: optimistic-locking `version` and auditing fields round-trip through the mapper exactly as before.
  - **Follow-up (code review, same ticket): closed the remaining `org.springframework.dao` leak.** `IssuanceR2dbcAdapter.insert`/`save` now map `OptimisticLockingFailureException` to the domain's `ConcurrentIssuanceUpdateException` at the infrastructure boundary, so `IssuanceServiceImpl` and `CredentialExpirationScheduler` no longer import a Spring Data-access type — `domainShouldNotDependOnSpringData` now also blocks `org.springframework.dao..`. Testcontainers seed helpers (`BitstringStatusListControllerRevokeIT`, `RevocationInstructionListenerIT`, `RevocationInstructionSingleTenantIT`) switched from `issuancePort.save` to `issuancePort.insert` to match production's `saveIssuance` path, and `IssuanceR2dbcAdapterTest` now asserts `insert`/`save` are mutually exclusive and covers the new exception mapping. No behavior change.
  - **Correction (EUD-168, found after this branch merged the change above): "no behavior change" did not hold for `holder_cnf`.** That field was added to `Issuance` by EUD-168 on a sibling branch and never carried over into the new `IssuanceEntity`/`IssuanceMapper` pair, so it silently stopped persisting — see the EUD-168 entry under Fixed below for the actual regression and fix.

### Added

- **Grant type persistence in OIDC4VCI issuance flow**: The `Issuance` entity now persists the original `grant_type` (e.g., `pre-authorized_code`, `authorization_code`) to allow the credential offer refresh flow to reuse the correct authorization method. This fixes an issue where refreshed offers always defaulted to `authorization_code`, breaking PIN-based flows. Includes DB migration `V12__Add_grant_type_to_issuance.sql`.
- **Remote signature error mapping (502)**: Added support for mapping 5xx errors from the QTSP during credential listing to `RemoteSignatureException`, providing more precise error causes.
- **`value_map` now propagates through `/.well-known/openid-credential-issuer`**: `CredentialProfile.ClaimDefinition` didn't declare a `value_map` field, so Jackson silently dropped it when deserializing credential profiles even though the source profile JSON already carried it (e.g. `gx.labelcredential.w3c.2`'s `gx:labelLevel` claim). The issuer metadata endpoint now serves it unchanged, letting clients map raw enum values (`P`) to their display label (`Professional`) without needing a separate, locally-bundled copy of the schema.
...
- **EUD-225 — Automatic credential revocation from a message-queue instruction (FR-11)**: an external client system can now publish a revocation instruction to a RabbitMQ queue and have the Issuer apply it with the exact same domain effect as an operator-initiated revocation (status change, status list publication, audit trail) — no human in the loop. Disabled by default (`issuer.messaging.revocation.enabled=false`, AD-6): a deployment with this Story merged behaves identically to before unless explicitly opted in.
  ...
  - **CI: fixed the SonarCloud Reliability Rating gate (rule `java:S8745`)**: `RevocationInstructionListenerIT` and `RevocationInstructionSingleTenantIT` each declared their setup across multiple `@BeforeEach`-annotated methods, whose relative execution order JUnit 5 does not guarantee within a single class. Consolidated each into one `@BeforeEach` calling the (now private) setup steps in an explicit, documented order — no behavioral change, both classes pass unchanged.
  - **R1 (targeted re-verification, found after the pass above): `auditAttempted` had its own residual of the same forged-field vulnerability** — it wrapped `formatDetails()`'s output in a second, literal `details="{}"` quoting layer that didn't compose with `formatValue`'s own escaping, so a value needing its own quotes closed the outer one early and leaked as bare top-level fields. `auditSuccess`/`auditFailure` never had this second layer; `auditAttempted` now matches them exactly. Deployment-hardening items from the same re-verification pass (broker TLS certificate validation depth, the `allow-insecure-transport` escape hatch also weakening the `guest` credential rejection, DLQ queue bounds, revocation-counter tag cardinality on the failure path) are deliberately deferred — see `docs/EUD-6-revocar-credencial/EUD-225/tech-debt.md` TD-4..TD-8 — because this integration is not provisioned for any client today (`enabled=false`, nothing in `eudistack-platform-iac`) and is meant to demonstrate the capability for tender purposes; the broker-hardening work is scoped to whenever a real customer integration is undertaken with infra.
  - **PR review findings (GitHub Advanced Security / CodeQL + Copilot code review, found after the PR was opened):**
    - **Log injection (`java/log-injection`, 3 instances) in `RevocationWorkflow.revokeInternal`**: `issuanceId` reached several `log.info`/`log.debug` statements unsanitized. It is already boundary-validated as a well-formed UUID on the queue path (`RevocationInstructionMessageMapper`), but the operator HTTP path's `RevokeCredentialRequest.issuanceId` only had `@NotBlank` — no format constraint — so a crafted value (e.g. containing a newline) could forge a fake log line via that path. Added a `@Pattern` UUID-format constraint to `RevokeCredentialRequest` (closing the gap at the boundary, matching what the queue path already enforces and what the `Issuance.issuanceId` column type actually is) and, defense-in-depth, quote/escape the value for every log statement in `revokeInternal` — the raw value is kept for the actual lookup/audit calls, which need the exact identifier. New `RevocationAuditDetails.quoteIfNeeded` (quotes and escapes an embedded `=`/`"`, on top of `sanitize`'s control-character stripping) — plain `sanitize` alone does not stop a value from forging extra `key=value` fields in a non-audit log line the way `AuditServiceImpl.formatValue` already does for the AUDIT sink; CodeQL's continued flagging on this exact call site after an initial `sanitize`-only fix is what surfaced the distinction.
    - **Potential infinite reclaim loop (Copilot, HIGH) in `R2dbcRevocationInstructionInbox.resolveExistingClaim`**: the Java-side freshness check (`claimedAt.isAfter(leaseThreshold)`) and the SQL reclaim condition (`claimed_at < :leaseThreshold`) disagreed at the exact boundary — a row with `claimed_at` equal to the threshold was neither "still fresh" nor "reclaimable", so `reclaimExpired` would update 0 rows and recurse back into `resolveExistingClaim`. Changed the Java-side check to `!claimedAt.isBefore(leaseThreshold)` so the two conditions are now complementary and mutually exclusive, eliminating the gap.
    - **Silent instruction drop under a real multi-replica race (Copilot, found on re-review) in the same method**: `readStatus`'s `.one()` completes empty (not an error) when zero rows match, reachable if a concurrent `release()` — a sibling replica retrying the same redelivered message, AC-09 — deletes the `IN_PROGRESS` row between this method's failed `INSERT ... ON CONFLICT DO NOTHING` and its own `SELECT`. Before this fix, `claim()` itself would complete empty (no `ClaimResult`, no error), and the caller could treat a dropped revocation instruction as silently handled — exactly the failure NFR-R-02 exists to prevent, and reachable specifically in the multi-replica deployment this Story's idempotency design targets. Fixed by retrying `claim()` from scratch (`switchIfEmpty`) when the read comes back empty: the row being gone is the same precondition a fresh claim already knows how to handle.
    - **Log injection (`java/log-injection`) in `AuditServiceImpl.auditSuccess`/`auditFailure`/`auditAttempted`**: `userId`/`resourceType`/`resourceId`/`reason` were interpolated raw, bypassing the exact same escaping (`formatValue`) already applied to `details` map values one parameter over — a gap CodeQL surfaced on the `auditAttempted` line this PR's R1 fix had just touched, but which existed identically in the two sibling methods. All three now route every named field through `formatValue`, not only the details map.
    - **Unbounded recursion in `claim`/`resolveExistingClaim`/`reclaimExpired` (independent code review, MEDIUM)**: the two recovery cycles (lost-race-on-expired-lease, and the vanished-row fix above) re-enter each other only after losing a race to a concurrent claim/release on the same row — not a tight synchronous loop, since each hop is a fresh async DB round-trip, but formally unbounded and undocumented. Added a bounded attempt counter (`MAX_CLAIM_ATTEMPTS = 5`, checked once at `resolveExistingClaim`'s single fan-in point for both cycles) and a new `RevocationInstructionClaimExhaustedException` (unclassified, so retried at the AMQP level like any other unexpected failure) for the pathological case of repeated collisions on one `messageId`.
...
- **EUD-167 (H1, security-high) — `SignDocServiceImpl` never cryptographically verified the signature the QTSP produced**: `verifyAndBuild` only compared the returned payload against the requested one, never the signature, the algorithm, or the certificate chain. Unlike the sibling `signHash` path (`JwsSignHashServiceImpl`), `signDoc` lets the QTSP build the whole JWS header itself, so nothing local could be assumed trustworthy. Demonstrated empirically by two adjacent, already-fixed defects in the sibling path that a real verification step would have caught immediately: #190 (QTSP ECDSA signatures returned in raw DER instead of JOSE R\|\|S, unverifiable by any conformant reader) and #191 (the self-signed trust anchor shipped inside `x5c`, violating HAIP §6.1/§6.1.1). `verifyAndBuild` now also verifies, in order: the algorithm is in a closed allowlist (`ES256/384/512`, `RS256/384/512`, `PS256` — mirrors `JadesHeaderBuilderServiceImpl.mapOidToJwtAlg`'s output set, closing algorithm-confusion attacks); `x5c` is present; the leaf certificate in `x5c` is byte-identical to the certificate `getCredentialInfo` already returned for this exact `credentialId` (the QTSP signed with the credential actually requested, not a substituted one); and the signature verifies cryptographically against that certificate's public key (`ECDSAVerifier`/`RSASSAVerifier`, same pattern already used by `VerifierServiceImpl`). Full X.509 chain-of-trust validation to a root CA and revocation checking (OCSP/CRL) are deliberately out of scope — no trust-anchor infrastructure exists in this codebase yet; tracked as new tech debt (see `docs/EUD-33-entrega-credenciales-delivery/EUD-167/tech-debt.md`, `eudistack-platform-dev`). `SignDocServiceImplTest` rewritten against real, self-signed EC P-256 test certificates (new `testImplementation` on `bcpkix-jdk18on`, needed to build them) instead of a literal placeholder string, with new regression cases for a disallowed algorithm, a missing `x5c`, a certificate that doesn't match `getCredentialInfo`, and a forged signature.

- **EUD-167 (H2, security-medium) — `POST /issuances` idempotency cache allowed replay across organizations of the same tenant**: `IdempotencyFilter`'s cache key was `tenantScope:idempotencyKey`, with no component identifying the calling organization. Two different organizations of the same tenant reusing (or guessing) the same `X-Idempotency-Key` value would collide on the same cache entry, and the second caller received the first caller's cached response verbatim — including a directly-delivered signed credential, which is never persisted anywhere else (EUD-167 finding M3). Found by the retroactive `/code-review` security audit of EUD-167. `IdempotencyFilter` now resolves the caller's organization from the `Authorization` bearer token (`AccessTokenService.getOrganizationId`) and includes it in the cache key (`tenantScope:organizationId:idempotencyKey`); a request whose token cannot be resolved skips idempotency caching entirely rather than risk a wrong key, and is rejected downstream on its own merits regardless.
  - **Follow-up (code review W3, found on re-review): the organization id came from an unverified parse of the raw JWT.** The filter read it straight from the `Authorization` header without checking the signature, safe only because this filter's `@Order` happens to run after Spring Security's chain today — a comment noted the filter "does not depend on its position vs. the auth chain," which was true of the request's authentication but not of this specific read. `IdempotencyFilter` now resolves the organization from the already-authenticated `SecurityContext` (`AccessTokenService.getOrganizationIdFromCurrentSession`, the same pattern `IntakeCallerAuthorizationFilter` already uses) instead of re-parsing the header itself, so the cache key can never be influenced by an unverified token regardless of filter ordering.
- **EUD-167 (TD-05) — bootstrap issuance answered an invalid delivery mode with a generic `500` instead of `400 invalid_request`**: `keepOnlyOid4vciDeliveryModes` threw a plain `IllegalArgumentException`, which has no `@ExceptionHandler` registered anywhere in `IssuanceExceptionHandler` — unlike the authenticated path (`resolveAndValidateDeliveryModes`), which already catches the same case and re-throws `InvalidDeliveryModeException` (mapped to 400). Also found while adding regression coverage: `issueCredentialWithoutAuthorization` called that method as a plain synchronous statement before constructing any `Mono` — safe only by accident, because its one caller (`BootstrapController`) happens to invoke it inside `Mono.deferContextual`. Wrapped the call in `Mono.fromCallable` so the exception always surfaces as a normal reactive error signal regardless of how a future caller invokes this method. New regression tests for a non-OID4VCI-only mode and a completely unknown delivery value.

- **EUD-167 (M1) — a status list entry reserved for a direct issuance was never released if signing or persistence failed afterward**: `performDirectIssuance` reserves the entry before signing (the status pointer has to be embedded in the payload that gets signed), but nothing freed it back when the operation failed downstream — every such failure permanently wasted one status list index for an `issuanceId` nothing would ever reference again. `StatusListProvider`/`StatusListWorkflow` gain `releaseEntry(issuanceId)` (deletes the `status_list_index` row by `issuanceId` — allocation is keyed the same way, so no re-signing of the status list credential is needed); `performDirectIssuance` now releases the entry, best-effort, whenever signing or persistence fails, without ever replacing or masking the original failure (a release failure is logged, not surfaced). New tests at all three layers (`BitstringStatusListProviderTest`, `StatusListWorkflowTest`, `IssuanceWorkflowImplTest`), including a case where the release itself fails and the caller still sees the original signing error.
  - **Follow-up (code review W1, found on re-review): the release above still missed one stage.** `onErrorResume` only wrapped `signCredential → saveIssuance`; `injectCredentialStatus` ran right after `allocateEntry`, outside that resume, so a malformed `enrichedDataSet` there threw past it and leaked the same status-list index M1 set out to stop leaking. `allocateEntry`, the inject step and `signCredential → saveIssuance` are now one chain under a single `onErrorResume`; `releaseStatusListEntryBestEffort`'s delete is a no-op when nothing was allocated, so covering an `allocateEntry` failure too is harmless.
  - **Follow-up (code review W2, found on re-review): `SignDocServiceImpl.buildVerifier` broke H1's closed error contract on a key/algorithm mismatch.** A leaf certificate whose key type disagrees with the JWS algorithm family (e.g. an RSA leaf under an ES256 header) threw `ClassCastException`, and a malformed verifier key or an invalid-Base64 `certInfo.certificates()` entry threw `IllegalArgumentException` — neither was caught by the existing `catch (JOSEException)`, so both surfaced as an unmapped 500 instead of `SignatureProcessingException`. Not a verification bypass (the request still failed either way). Both are now caught and mapped; `SignatureProcessingException` gained a `(message, cause)` constructor to keep the original exception as the cause.

- **`delivery` bounded to letters/comma/whitespace, max 64 chars (tech-debt TD-06).** Blocks a CRLF/control-character value from reaching `DeliveryMode.parse` or the raw `log.error` interpolation in `IssuanceWorkflowImpl` — rejected at the DTO boundary (400) instead. `holder_key` left as-is: its security-relevant part (the `jwk`) is already explicitly size-bounded (`HolderKey.MAX_JWK_NODE_BYTES`, 8 KB) before Nimbus parses it. New `IssuanceRequestValidationTest` + an end-to-end `IssuanceControllerTest` case.
...
- **`responses[]` no longer emits an empty `body: {}` for an email-only dispatch (code-review B1, follow-up to the entry below).** `CredentialOfferServiceImpl` only builds a `credential_offer_uri` when the requested modes include one that `returnsUri` (`ui` does, `email` alone does not); `IssuanceHttpEnvelopeMapper` was attaching a `ChannelBody` unconditionally for any non-`direct` channel regardless of whether there was a URI to put in it. `body` is now `null` (dropped from the JSON) rather than an uninformative empty object when a channel genuinely has nothing to report. Corrected `EUD-167/acceptance-criteria.md`'s D-6 body-shape section (`eudistack-platform-dev`) to match. New `IssuanceHttpEnvelopeMapperTest`.

- **`email` no longer echoes `credential_offer_uri` in its own `responses[]` item when dispatched alongside `ui` (found after the fix above shipped, reported by Roger Miret).** The B1 fix above still attached the shared offer URI to `email`'s body whenever `ui` was also requested — reasoned at the time as "it does have the URI to report". On reflection the API caller does not need it back through the `email` item: the URI was already embedded and delivered inside the email itself, not returned to the caller through that channel of the response. `email`'s item now never carries a `body`; only `ui`'s does. Corrected `EUD-167/acceptance-criteria.md`'s D-6 body-shape section again to match. `IssuanceHttpEnvelopeMapperTest` updated accordingly.

- **BREAKING — `POST /api/v1/issuances` response body restructured: `responses[]` replaces the flat `signed_credential`/`credential_offer_uri`/`delivery_results` fields (EUD-167 D-6).** D-5 sketched an envelope of `channel`/`status`/`body`-or-`error` per requested delivery channel but never pinned down what `body` contains; this fix makes that explicit and implements it. New shape: `{ "responses": [ { "channel": "direct"|"ui"|"email", "status": <int>, "body": { "signed_credential": "..." } | { "credential_offer_uri": "..." }, "error": { "type", "title", "status", "detail" } } ] }` — `body` only on success (`status: 200`), `error` (RFC 9457) only on failure (`status: 503`, or `504` for a timed-out wallet leg). The line-level HTTP status (200/207/500) logic from the earlier fix in this same section is unchanged; only the body shape changed. `IssuanceHttpEnvelopeMapper` (new, infrastructure layer) projects the untouched internal `IssuanceResponse`/`DeliveryResult` onto the new `ChannelBody`/`ChannelError`/`ChannelResponse`/`IssuanceHttpResponse` DTOs at the controller boundary — `IssuanceWorkflowImpl` itself did not change. No external consumer of the previous shape has shipped yet.

- **EUD-168 (AD-11 superseded by EUD-167 D-6/D-5) — a hybrid issuance whose direct mode failed while the wallet mode dispatched, or vice versa, no longer returns 500/200: it returns `207 Multi-Status`.** AD-11 (2026-08-31) already isolated the direct leg from the wallet leg's `Mono.zip` (a direct failure no longer discards an already-dispatched wallet result), but mapped that outcome to HTTP 500 whenever the *direct* mode failed and to 200 whenever it didn't — decided before EUD-167's D-5 (2026-09-04) fixed the general contract: any mix of a completed channel and a failed one is `207 Multi-Status` (RFC 4918 §11.1/§13), regardless of which channel is which. `IssuanceController.toResponseEntity` now classifies by whether *any* requested channel succeeded/failed instead of singling out the direct mode. Net effect: a wallet-mode failure with the direct mode delivered, previously a silent 200, is now also 207 — the client MUST inspect `delivery_results[]` in both directions. A request where every channel failed (e.g. a wallet-only issuance with the wallet dependency down) is not a mixed outcome and now returns 500 instead of silently 200/202 (this path only reaches the controller when the direct mode was never attempted, so the workflow's own re-raise-on-total-failure guard does not catch it — untested before this fix). See `eudistack-platform-dev`'s `docs/EUD-33-entrega-credenciales-delivery/EUD-167/spec-deltas.md` D-6 for the full reconciliation between the two Stories' decisions.

- **EUD-168 (AD-9) — the merge of EUDISTACK-650 (H-05) silently stopped persisting `holder_cnf`, breaking every Wallet-mode issuance of the two AD-8 exempt machine-credential types**: AD-9 added `holder_cnf` to the `issuance` row so the machine LEARCredential's `cnf` (sourced from the request's `holder_key`, since these types declare no `proof_types_supported` and no key proof will ever arrive) survives from the issuance request to the later, separate request at the OID4VCI Credential Endpoint. EUDISTACK-650 (H-05, merged into this branch afterward) split the domain `Issuance` from its R2DBC persistence model into `IssuanceEntity` + `IssuanceMapper`, and round-tripped every field that existed in `Issuance` at the time — except `holderCnf`, added by this Story on a sibling branch and never back-ported into the new mapper. The domain object still carried a stray, dead `@Column("holder_cnf")` (removed here — H-05 already documents `Issuance` as framework-free) which made the class read as if it were still directly persisted. Net effect: the direct-delivery leg was unaffected (it signs synchronously from the in-memory `cnf`, never reading the row back), but every Wallet-mode (`ui`/`email,ui`) issuance of `learcredential.machine.sd.1`/`learcredential.machine.w3c.3` failed at redemption with `IllegalStateException: Missing cnf (expected kid/jwk/x5c)` (HTTP 500 at `/oid4vci/v1/credential`), confirmed against the persisted rows (`holder_cnf` was `NULL` for every issuance of these types, including the ones that reached `VALID` via the direct leg). Fixed by adding `holder_cnf` to `IssuanceEntity` and wiring both directions of `IssuanceMapper`. `IssuanceEntity`, `IssuanceMapper`, `Issuance`.

- **EUD-168 (FR-11) — a failed direct mode discarded the whole multi-mode response, wallet offer included**: `executeIssuanceForModes` isolated the wallet leg but not the direct one, so a direct failure aborted the `Mono.zip` and the response with it — the operator got a 500 with no indication that the credential offer had already been created and emailed. Inside the wallet leg the same shape existed one level down: a failed email send was propagated as an exception, which marked the `ui` mode failed too and threw away a URI that had been built correctly. Now every declared mode is reported on its own (`delivery_results`), a mode that completed stays reported as completed, and the `credential_offer_uri` of a dispatched wallet mode survives in the body. The HTTP status answers a different question from the body: **5xx when the direct mode failed** (the operator asked for a credential on screen and has none) while the body keeps the per-mode results and the URI; success otherwise. When the direct mode fails and *nothing else* was delivered there is no partial outcome to report, so the original error is re-raised and rendered as its own problem detail rather than flattened into a generic 500. `CredentialOfferResult` gains `emailError` — the offer service reports the failure instead of throwing, and each caller decides: an issuance records it on the `email` mode, while `POST /credential-offer-refresh` (whose only job is that email) still turns it into an error, unchanged. See `docs/EUD-33-entrega-credenciales-delivery/EUD-168/technical-design.md` §3.5 AD-11.

- **EUD-168 — direct delivery signed with `X-Id-Token`, an optional header, so every direct issuance of a profile that does not require it failed with `NullPointerException: token cannot be null`**: `IssuanceController` passed the `X-Id-Token` header as the caller token for the direct delivery mode, which signs inside the issuance request. That header is only required by the VerifiableCertification profile (`requiresIdToken`), so every other type arrived with `null` and died in `BitstringStatusListProvider.allocateEntry`'s precondition — in a hybrid delivery, *after* the wallet offer had already been created and dispatched. A pre-existing defect from EUD-167 (PR #130) that EUD-168 is the first to reach: before the schema ceiling no real credential type was eligible for direct delivery, so the path was never exercised with a profile that requires no `X-Id-Token`. `issueCredential` now takes `idToken` and `bearerToken` as separate parameters — the PDP keeps reading the identity assertion, the direct mode signs with the caller's `Authorization` header (the same token `revocationWorkflow.revoke` already uses in that controller, and the analogue of the OID4VCI access token the wallet modes sign with). See `docs/EUD-33-entrega-credenciales-delivery/EUD-168/technical-design.md` §3.5 AD-10.

- **EUD-168 — the startup invariant read an empty `proof_types_supported`/`cryptographic_binding_methods_supported` as absence rather than as the incoherence EC-01 requires**: a profile declaring `proof_types_supported: {}` alone loaded silently, because the invariant only compared the two OID4VCI fields against each other and an empty object equals absence under that comparison. Added a third, unconditional check that fires independently of that comparison and fails the profile's load, naming the profile and the offending field. `CredentialProfileBindingInvariant`.

- **EUD-168 (`/code-review` remediation) — `holder_key` validation and its binding into the signed credential's `cnf`/`mandatee.id` hardened for the two AD-8 exempt machine-credential types**: earlier hardening validated the JWK content (rejecting private keys, `oct`, and — since this pass — anything but EC P-256) and derived the holder's `did:key` from it, but left two gaps a full audit found: (1) `kid` and `x5c` — meaningful only on the OID4VCI proof path, where a wallet's own key proof backs them — carried no key material this path could verify, yet were accepted for `holder_key` too, unvalidated and unbound; `holder_key` is now required to be the `jwk` form for these two types. (2) The signed `cnf` is now Nimbus's own canonical serialization of the JWK, not the raw request body, so an unrecognized member alongside a structurally valid key no longer rides along into the credential. (3) `DidKeyDerivation` (used on both the direct and wallet-mode signing paths) falls back to a random identifier when it cannot decode a JWK's coordinates — safe on the OID4VCI proof path it was written for (an already-verified key), but reachable here before this pass via a non-P-256 `holder_key`; both paths now discard anything that isn't an actual `did:...` before binding it into `mandatee.id`, on top of the P-256-only restriction that makes the fallback unreachable in practice. (4) A repository failure while looking up the email-delivery recipient could still reach the client as a raw exception message in `delivery_results[].error` (an R2DBC error can name a table/schema, and the schema name is the tenant); it now goes through the same safe constant the email-send step already used. `HolderKey`, `IssuanceWorkflowImpl`, `Oid4VciCredentialWorkflowImpl`, `CredentialOfferServiceImpl`.

- **EUD-168 (TD-09, `/code-review` re-verification, 2026-09-01) — no correlatable signal when a `DidKeyDerivation` fallback silently dropped the `cnf`-`mandatee.id` binding**: `IssuanceWorkflowImpl.holderDidFromCnf` and `Oid4VciCredentialWorkflowImpl.resolveHolderDid` already discarded a derived holder DID that wasn't `did:...`-prefixed (the guard from the entry above), but the only trace left behind was `DidKeyDerivation`'s own operational `log.warn` about the decode failure — nothing naming the consequence, and indistinguishable from a routine WARN. Both call sites now emit a dedicated `AuditService.auditFailure` event (`credential.holder_did.derivation_fallback`) carrying `processId`/`issuanceId`, best-effort (a broken audit channel never fails an issuance that would otherwise succeed). D1/D2's fixed-length coordinate canonicalization (`HolderKey.validateAndCanonicalizeJwk`) already make this branch unreachable for a `holder_key` that went through validation — this is defensive observability for a future Nimbus version or a second, unvalidated caller of `DidKeyDerivation`, not a functional change. See `docs/EUD-33-entrega-credenciales-delivery/EUD-168/tech-debt.md` TD-09.

- **EUD-168 (TD-07) — the schema delivery ceiling had unit coverage but no end-to-end test against the real HTTP endpoint, and the fail-fast startup invariant had never been exercised against the real vendored `legacy/` profiles**: closes the gap flagged back in `[3.6.x] Fixed - 2026-07-30` (`IssuanceController_IT` postponed) and tracked since as TD-07. New `DirectDeliveryCeilingTest` (`issuance/infrastructure/it`, despite the package name a plain `*Test`, no Testcontainers) drives `POST /api/v1/issuances` through a hand-wired `IssuanceController`/`IssuanceWorkflowImpl` with `SchemaDeliveryCeiling` and `CredentialProfileRegistry` real (loaded from the classpath fixture) and only the true I/O boundaries — signing, status list reservation, persistence, the Wallet offer trigger — mocked: a `direct` request for a bound type never reaches any of those four mocks (AC-03), the 409 body is the real, actionable one produced by `SchemaDeliveryCeiling`/`IssuanceExceptionHandler` (ES-03), and an unknown delivery mode is rejected `400 invalid_request` before the ceiling is even evaluated (ES-01). `CredentialProfileRegistryTest` gains `shouldLoadTheThirteenRealVendoredProfilesWithoutViolatingTheInvariant`, pointing a real `PathMatchingResourcePatternResolver` at the vendored `dev-tools/credentials/profiles` copy (not the classpath fixture the rest of the file uses) to confirm all 13 real profiles — 7 current plus the 6 under `legacy/` — load without tripping `CredentialProfileBindingInvariant` (EC-07). See `docs/EUD-33-entrega-credenciales-delivery/EUD-168/tech-debt.md` TD-07.

- **EUD-168 (D-4, doc-only) — `V12__Add_holder_cnf_to_issuance.sql`'s header comment still described `holder_cnf` as accepting `{"jwk"|"kid"|"x5c": ...}` and named the route `POST /v1/issuances`**: both were stale — `HolderKey.fromJson` has required the bare `jwk` member since `c2086cc3`/`0599055a` (F1a/D3, `kid`/`x5c` dropped as unverifiable on this path), and the route has always been `/api/v1/issuances`. Comment-only, no behavior change.

- **Employee v4 credentials could be issued with fields outside the `mandatee`/`mandator` schema**: the shared JSON schema (`learcredential.employee.w3c.4.json`, vendored here from `eudistack-platform-dev`) allowed `additionalProperties` on `mandate.mandatee`/`mandate.mandator`, so `PayloadSchemaValidatorImpl` let through payloads with misnamed or extra fields (e.g. `mandator.emailAddress` instead of `mandator.email`). This is a regression from the `LEARCredentialEmployee` typed-DTO removal (dynamic-parsing migration) — the same defect was fixed once in the old typed model and reopened when that guardrail was replaced by schema validation. Tightened to `additionalProperties: false` and added `mandatee.id`/`mandator.id` (already used in practice, previously undeclared) to `properties`. Added `PayloadSchemaValidatorImplTest` (none existed for this validator before).
- **Grant Type constant correction**: Corrected the `PRE_AUTHORIZATION_CODE` constant to `PRE_AUTHORIZED_CODE` to match the OIDC4VCI standard value `pre-authorized_code`, improving compatibility with the OIDC4VCI ecosystem.
- **Unit test refactoring**: Cleaned up redundant `eq()` argument matchers in various workflow implementation tests (`IssuanceWorkflowImplTest`, `CredentialOfferRefreshWorkflowImplTest`), improving test code readability.
- **EUD-215 — `/oid4vci/v1/notification` rejected every spec-conformant call with `400`**: OID4VCI 1.0 FINAL §11.1 names the request fields `notification_id`/`event_description` (snake_case), but `NotificationRequest` had no `@JsonProperty` mapping — Jackson only matched the literal Java field name (`notificationId`), so a conformant request deserialized it as `null`, failing `@NotBlank` validation with `400`. Caught by the OIDF conformance suite's `oid4vci-1_0-issuer-happy-flow` test (`OID4VCI-1FINAL-11.2 EnsureHttpStatusCodeIs2xx`). Went unnoticed because our own Wallet PWA sent the same non-conformant camelCase key this record always expected (companion fix in `eudistack-core-wallet-pwa`). Added `@JsonProperty` for the spec-correct snake_case keys, plus `@JsonAlias` for the old camelCase keys so an un-updated wallet build doesn't suddenly start failing.
- **EUD-215 — the Token Status List (`draft-ietf-oauth-status-list`) `lst` claim used the wrong compression, undecodable by any conformant reader**: `TokenStatusListCredentialFactory.buildUnsigned` reused `encodedList` (the shared bitstring storage format — multibase + base64url + **GZIP**, RFC 1952) as-is for the `lst` claim. §4.1 of the draft mandates "DEFLATE [RFC1951] with the ZLIB [RFC1950] data format" — raw DEFLATE inside the 2-byte zlib header + Adler-32 trailer, which is neither GZIP nor bare RFC 1951. Caught by the OIDF conformance suite's `oid4vci-1_0-issuer-happy-flow` test (`OTSL-6.2 CheckCredentialStatus`: `DataFormatException: incorrect header check`). Now decodes back to the raw bitstring (`BitstringEncoder`) and re-compresses with zlib-framed DEFLATE (`new Deflater(BEST_COMPRESSION)`, i.e. `nowrap=false`) before base64url-encoding — the W3C Bitstring Status List JWT (which does use GZIP) is unaffected, it still reads `encodedList` directly. A first attempt at this fix went to header-less DEFLATE (`nowrap=true`), which reproduced the exact same `incorrect header check` on the reader side; the accompanying test now inflates with the default `Inflater` (zlib) rather than `new Inflater(true)`, which accepted both framings and masked the bug.
- **EUD-215 — both status list endpoints served any list regardless of its format, so a wallet following `status.status_list.uri` got a W3C VC it could not decode**: `/token/v1/credentials/status/{id}` (`application/statuslist+jwt`) and `/w3c/v1/credentials/status/{id}` (`application/vc+jwt`) read the same `status_list` table, and `getSignedStatusListCredential` returned the stored blob without consulting the row's `format`. On STG, `GET /issuer/token/v1/credentials/status/2` answered `200` with a `Content-Type: application/statuslist+jwt` carrying a `BitstringStatusListCredential` (`typ: vc+jwt`) — byte-identical to what the W3C endpoint served for the same id. Caught by `OTSL-6.2 CheckCredentialStatus`. A list is now only reachable through the endpoint matching its own format; a mismatch is a `404` (`StatusListNotFoundException`), never a `200` under the wrong media type.
- **EUD-215 — an issuance whose status list entry was already allocated on a list of another format got a status reference that could not resolve**: `BitstringStatusListProvider.findExistingAllocation` looked the allocation up by `issuance_id` and then built the entry URL from the *requested* format, ignoring the format of the list the index actually sits on — producing a `/token/v1/...` URI pointing at a `bitstring_vc` list. Now verifies the stored list first and fails with the new `StatusListFormatMismatchException` instead of emitting a credential whose status is unresolvable.
- **EUD-215 — `/oid4vci/v1/credential` derived the credential format from the stored issuance row instead of the profile, silently downgrading SD-JWT issuance**: `Oid4VciCredentialWorkflowImpl.enrichAndSign` read `proc.getCredentialFormat()` (falling back to `jwt_vc_json`), but the wallet asks by `credential_configuration_id` and the Credential Issuer Metadata advertises `profile.format()` for it — so the profile is the authority. An offer row written before its profile carried a format, or before that profile moved to `dc+sd-jwt`, kept `credential_format` NULL or `jwt_vc_json` and took the whole chain down the W3C branch: `BITSTRING_VC` for the status list allocation instead of `TOKEN_JWT`, and a W3C `credentialStatus` object instead of `status.status_list` inside an SD-JWT VC. Matches the observed STG state, where the `sandbox` tenant has only `bitstring_vc` lists (ids 1–2, nothing above) despite `eu.europa.ec.eudi.pid.1` being published as `dc+sd-jwt`. The profile now wins, with a `WARN` when the stored row disagrees.
- **EUD-215 — `/oauth/token` rejected a missing/invalid PKCE `code_verifier` with `invalid_request` instead of the `invalid_grant` RFC 7636 §4.6 mandates**: a prior fix (EUD-215, PKCE/DPoP error shape) caught `PkceVerifier`'s and `DpopValidationService`'s `IllegalArgumentException`s in one shared `try/catch` and mapped both to `OAuthTokenException.invalidRequest(...)` — correct for a missing/invalid DPoP proof, but RFC 7636 §4.6 specifically requires `invalid_grant` for a `code_verifier` that fails PKCE verification. Caught by the OIDF conformance suite's `fapi2-security-profile-final-ensure-pkce-code-verifier-required` test (`CheckErrorFromTokenEndpointResponseErrorInvalidGrant`: got `invalid_request`, expected `invalid_grant`). Split `TokenServiceImpl.validateAndBuildAuthCodeToken`'s single combined `try/catch` into two separate ones — PKCE failures now map to `OAuthTokenException.invalidGrant(...)`, DPoP failures keep `invalidRequest(...)`.
- **EUD-215 — `/oauth/par` silently accepted a pushed authorization request that itself carried a `request_uri` parameter**: RFC 9126 §4 defines `request_uri` as the PAR endpoint's own *response* value — it must never be accepted as an *input* of the pushed request. `PushedAuthorizationRequest` had no field for it, so Spring's form binder silently dropped the parameter and `ParServiceImpl` never knew it was present, returning a normal `201` instead of rejecting the request. Caught by the OIDF conformance suite's `fapi2-security-profile-final-par-authorization-request-containing-request_uri-form-param` test. Added the field (bound only for detection, never otherwise read) and an explicit check rejecting with `OAuthTokenException.invalidRequest(...)` (`400 invalid_request`) when present, alongside the existing `response_type`/`redirect_uri`/PKCE checks.
- **EUD-215 — `/oauth/token` returned our internal Problem-Details body instead of the RFC 6749 §5.2 `{error, error_description}` shape when PKCE or DPoP validation failed on an `authorization_code` grant**: `TokenServiceImpl.validateAndBuildAuthCodeToken` called `PkceVerifier.verifyS256` and `DpopValidationService.validate` directly — both raise plain `IllegalArgumentException` on failure (e.g. `"Missing DPoP proof"`), which `Oidc4vciExceptionHandler`'s generic handler maps to `GlobalErrorMessage` (`{type, title, status, detail, instance}`, no `error` field) instead of `OAuthErrorResponse`. Caught by the OIDF conformance suite's `fapi2-security-profile-final-ensure-holder-of-key-required` test (`CheckTokenEndpointReturnedInvalidRequestGrantOrDPopProofError`: the response was a `400` but had no `error` field to check). Same gap `ParServiceImpl` and `AuthorizationServiceImpl` already had fixed for their own PKCE/DPoP/response_type checks — both calls are now wrapped in a `try/catch` translating `IllegalArgumentException` into `OAuthTokenException.invalidRequest(e.getMessage())` (`400 invalid_request`). PKCE failures had the identical defect (confirmed via a pre-existing unit test that asserted the *wrong*, unwrapped behavior) but no conformance test had caught it yet — fixed alongside DPoP rather than leaving a known-identical gap for a future test to rediscover.
- **EUD-215 — `/oid4vci/v1/authorize` accepted authorization requests that skipped PAR entirely, despite the Issuer advertising `require_pushed_authorization_requests: true`**: `AuthorizationServiceImpl.authorize` only used `request_uri` to decide *how* to process the request (via `pushAuthorizationRequestAuthorization` vs `processDirectAuthorization`) — it never checked whether PAR was actually required before falling back to the direct path, so a client could bypass the Pushed Authorization Request step by hitting `/oid4vci/v1/authorize` with plain query parameters, contradicting the Issuer's own metadata (RFC 9126 §5: an AS that requires PAR MUST reject a non-PAR authorization request). Caught by the OIDF conformance suite's `fapi2-security-profile-final-ensure-unsigned-authorization-request-without-using-par-fails` test — a direct request without `request_uri` was accepted and redirected back with a valid code instead of being rejected. Now `authorize` rejects with `OAuthTokenException.invalidRequest("Pushed Authorization Request is required")` (`400 invalid_request`) before ever reaching `processDirectAuthorization`, whenever the profile's `authorizationCode().requirePar()` is `true`. **Copilot review finding:** the new check's `OAuthTokenException` shape clashed with `AuthorizationServiceImpl`'s pre-existing `response_type`/PKCE/invalid-`request_uri` checks, which still threw plain `IllegalArgumentException` — mapped by `Oidc4vciExceptionHandler` to a different body (`GlobalErrorMessage` instead of `OAuthErrorResponse`), so `/oid4vci/v1/authorize` returned an inconsistent error schema depending on which validation failed. Converted the remaining three throw sites to `OAuthTokenException.invalidRequest(...)` too, matching the same standardization `ParServiceImpl` already went through earlier in this Story (see the `redirect_uri` entry below).
- **EUD-215 — `/oid4vci/v1/par` accepted an authorization request missing `redirect_uri`, which then crashed `/oid4vci/v1/authorize` with a `500`**: `PushedAuthorizationRequest` has no bean-validation constraint on `redirect_uri` (it is legitimately absent from other grant shapes) and `ParServiceImpl.pushAuthorizationRequest` never checked for it explicitly, so a PAR request without one was accepted with `201`. The missing value only surfaced downstream, once `/oid4vci/v1/authorize` redeemed the PAR and `AuthorizationServiceImpl.buildRedirectUri` called `new StringBuilder(redirectUri)` with a `null` — an uncaught `NullPointerException` instead of a clean rejection. Caught by the OIDF conformance suite's `fapi2-security-profile-final-ensure-request-object-without-redirect-uri-fails` test, which requires the request to fail with `invalid_request` at the PAR endpoint (the only automatable one of the test's three accepted outcomes — the other two need a manually uploaded screenshot of an error page or a redirect to a pre-registered `redirect_uri`, neither of which this Issuer implements). Added an explicit `redirect_uri` check to `ParServiceImpl`, alongside the existing `response_type`/PKCE checks, rejecting with `OAuthTokenException.invalidRequest("redirect_uri is required")` (`400 invalid_request`) before the request is ever stored.
- **EUD-215 — `/token` crashed with a `500` on an `authorization_code` grant whose code has no `issuer_state`**: `issuer_state` is OPTIONAL at the PAR/authorize level (RFC 9126 / OID4VCI 1.0) — a wallet-initiated authorization request never sends one — but `TokenServiceImpl.validateAndBuildAuthCodeToken` unconditionally called `issuerStateCacheStore.get(codeData.issuerState())` to resolve the `issuanceId` an access token is bound to. Guava's `Cache.getIfPresent` throws `NullPointerException` on a `null` key, which the generic catch-all turned into an opaque `500 "An unexpected error occurred"` instead of a proper OAuth2 error. Caught by the OIDF conformance suite's `fapi2-security-profile-final-happy-flow` test. Now checked explicitly before the cache lookup and rejected with `OAuthTokenException.invalidGrant(...)` (`400 invalid_grant`) — this Issuer can only complete an `authorization_code` exchange for a code tied to an Issuer-initiated credential offer, so a missing `issuer_state` is unusable here even though the parameter itself is spec-optional. `TokenServiceImplAuthCodeTest` already covered the `authorization_code` grant's happy paths (PKCE, DPoP) and other failure modes (invalid code, redirect_uri mismatch, PKCE failure); none exercised a code with no `issuer_state`, so this adds that regression case to the same class.
- **EUD-215 — `/credential` failed to parse a request using the `credential_identifier` addressing mode, returning a generic body with no `error` field**: OID4VCI 1.0 §8.2 defines `credential_configuration_id` and `credential_identifier` as mutually exclusive ways to identify the requested Credential — `credential_identifier` is used only when the Token Response returned `authorization_details` with `credential_identifiers`, which this Issuer never does (only the scope-based flow is implemented). `CredentialRequest` had no `credential_identifier` field at all and marked `credential_configuration_id` as JSON `required = true` unconditionally, so a request using `credential_identifier` failed Jackson deserialization outright (`400 "Failed to read HTTP message"`, no `error` field) instead of being recognized and cleanly rejected. Caught by the OIDF conformance suite's `oid4vci-1_0-issuer-fail-unknown-credential-identifier` test. `credential_identifier` is now a recognized (nullable) field; since this Issuer can never resolve one, its mere presence is rejected with a new `UnknownCredentialIdentifierException` → `error="unknown_credential_identifier"` (OID4VCI 1.0 §8.3.1.2), checked before and independently of the `credential_configuration_id` guards below.
- **EUD-215 — `/credential` issued a credential for an unknown or mismatched `credential_configuration_id`, ignoring the request entirely**: `Oid4VciCredentialWorkflowImpl` computed whether the requested `credential_configuration_id` was registered (`knownRequestedConfigurationId`) but only ever used the result to decide what to log — the actual issuance always proceeded using `Issuance.getCredentialType()`, never checking it against what the client asked for. A request with a completely made-up `credential_configuration_id` got `200 OK` with a real, validly-signed credential; a request naming a *different but valid* configuration than the one the Issuance/token was actually authorized for (e.g. asking for PID with a LEAR Employee token) fared no better. Caught by the OIDF conformance suite's `oid4vci-1_0-issuer-fail-unknown-credential-configuration` test. Now rejects both cases with `UnknownCredentialConfigurationException`, mapped by a new `Oidc4vciExceptionHandler` entry (scoped override of the `SharedExceptionHandler` version used by the backoffice credential catalog) to `error="unknown_credential_configuration"` — the exact code OID4VCI 1.0 §8.3.1.2 defines for this case (an earlier version of this fix used `unsupported_credential_type`, not a recognized code at all, flagged by the conformance suite as non-standard). The unknown case is rejected before the Issuance is even loaded; the mismatch case only after, since it needs the Issuance's own type to compare against.
- **EUD-215 — `/credential` proof errors used our internal Problem-Details body instead of the OID4VCI 1.0 §8.3.2 `{error, error_description, c_nonce, c_nonce_expires_in}` shape**: `InvalidOrMissingProofException`/`ProofValidationException` were mapped to our internal `GlobalErrorMessage` (`type`/`title`/`detail`/`instance`, no `error` field at all); `ProofValidationException` didn't even issue a fresh `c_nonce`. Caught by the OIDF conformance suite's `oid4vci-1_0-issuer-fail-missing-proof` test (`VCIValidateCredentialErrorResponse`: required field `error` missing). New `CredentialErrorResponse` record backs both handlers now, mapping to `error="invalid_proof"` per spec and always including a fresh nonce. `/credential`'s error shape was explicitly called out as untouched in the PAR error-shape fix below — this closes that gap.
- **EUD-215 — client attestation JWTs missing the `sub` claim were silently accepted**: `ClientAttestationValidationService` extracted `client_id` from the WIA's `sub` claim but never checked it was present — with `sub` absent, `clientId` ended up `null` and validation continued anyway, so `PAR` returned `201 CREATED` with a `request_uri` instead of rejecting the request. Per `draft-ietf-oauth-attestation-based-client-auth`, `sub` identifies the OAuth client and is required. Caught by the OIDF conformance suite's `oid4vci-1_0-issuer-fail-client-attestation-no-sub` test. Now throws `IllegalArgumentException("Client Attestation JWT missing sub claim")` when `sub` is null/blank, which the existing PAR error mapping already turns into a proper `invalid_client` OAuth2 error.
...
- **EUD-215 — PAR error responses used our internal Problem-Details-style body instead of the RFC 6749 §5.2 `{error, error_description}` shape RFC 9126 §2.3 requires**: the `IllegalArgumentException` → 400 mapping added for PAR (see below) fixed the status code but not the body shape — `/token` already had a proper `OAuthTokenException` → `OAuthErrorResponse` path for this, but `ParServiceImpl` still threw raw `IllegalArgumentException`, which the generic handler serializes as our own `GlobalErrorMessage` (no `error` field at all). Caught by the OIDF conformance suite's `oid4vci-1_0-issuer-fail-invalid-client-attestation-signature` test (`CheckErrorFromParEndpointResponseErrorInvalidClientOrInvalidRequestOrInvalidClientAttestation`: 400 status was correct, but the required `error` field was absent). `ParServiceImpl` now throws `OAuthTokenException` (reusing the existing `/token` infrastructure) instead of `IllegalArgumentException`: `invalid_request` for response_type/PKCE/DPoP-proof failures, `invalid_client` for client attestation failures. `/credential`'s own error shape is untouched — this is scoped to the two OAuth2 endpoints (PAR, token) that RFC 6749 §5.2 governs.
...
- **EUD-168 — Direct delivery is decided by the schema, not by tenant configuration (FR-06, FR-02)**: this **replaces the previous EUD-168 entry**, which described the opposite capability — enabling direct delivery for any holder-bound type by supplying a `holder_key`. That rule is withdrawn.
  - **`proof_types_supported` is the single signal of holder binding** (ADR-110). Present ⇒ bound, the Credential Endpoint demands a key proof, and direct delivery is impossible — direct has no wallet and therefore no proof-of-possession. Absent ⇒ unbound and eligible for direct. `cryptographic_binding_methods_supported` (which describes how key material is represented, not whether it is required) and the proprietary `cnf_required` no longer decide anything. The Credential Endpoint now reads the same field as the issuance path, so the two cannot drift apart.
  - **`SchemaDeliveryCeiling` is a ceiling, not a default**: eligibility resolves as `tenant configuration ∩ schema ceiling`. A stored configuration that predates this rule and still lists `direct` for a bound type is intersected away rather than honoured, at issuance time and on the delivery-config write path alike. `GET /api/v1/backoffice/delivery-config/{id}` now returns `schema_eligible_modes` so a TenantAdmin sees why a mode is unavailable instead of discovering it through a rejected write.
  - **Profile invariants are checked at startup, not at first issuance**: `CredentialProfileBindingInvariant` fails the boot naming the `credential_configuration_id` and the offending field. Absence must be declared by removing a key, never by emptying it.
  - **Profiles migrated**: `gx.labelcredential.w3c.2`, `learcredential.machine.sd.1` and `learcredential.machine.w3c.3` drop both OID4VCI binding fields and become eligible for direct delivery. They also stop requiring a key proof in the wallet flow, so the label credential becomes a bearer attestation end-to-end — a product decision per type, not a technical adjustment.
  - **Scoped, temporary exception (AD-8)**: the two `learcredential.machine.*` types keep `cnf_required: true` and build their `cnf` from the `holder_key` in the issuance request, for **every** delivery mode rather than the direct one alone — with `proof_types_supported` gone, no key proof reaches the wallet flow either. `HolderKey` and the `holder_key` request field therefore survive, scoped to a closed list of two configuration ids instead of being a general capability. The key is mandatory for those types: a missing one is a rejected request, never an issuance that silently drops the `cnf`.
  - **The exempted `cnf` travels on the `issuance` row** (`holder_cnf`, `V12`): the wallet delivery modes sign in a *later* request than the one carrying the `holder_key` — the OID4VCI Credential Endpoint, where an exempted type receives no key proof either — so the key has to outlive the issuance request. Reading `bindingInfo.cnf()` alone there made `email`/`ui` delivery of `learcredential.machine.*` fail at signing with `Missing cnf (expected kid/jwk/x5c)` while the direct mode worked; the Credential Endpoint now falls back to the persisted value, with the key proof still winning whenever one arrives (cryptographic evidence before issuer assertion). Nullable column, populated only for the exempted types, holding public key material only.
  - **Known limitation**: that `holder_key` arrives with no proof of possession, so the resulting `cnf` is an issuer assertion rather than cryptographic evidence. It degrades nothing today — the Verifier's `client_credentials` flow already skips that binding check — but reactivating that check is a prerequisite for the claim to mean anything.
