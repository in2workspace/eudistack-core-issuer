...
public class VaultExportAdapter implements VaultExportPort {
    ...
    public Mono<byte[]> exportPrivateKey(LegacyKeyId keyId) {
        return vaultWebClient.get()
                ...
                .map(response -> {
                    Map<String, String> keys = (response == null || response.data() == null)
                            ? null
                            : response.data().keys();
                    ...
                    String highestVersion = keys.keySet().stream()
                            .filter(v -> v != null && v.chars().allMatch(Character::isDigit))
                            .max(Comparator.comparingInt(Integer::parseInt))
                            .orElseThrow(() -> new RuntimeException(
                                    "vault_export_empty_keys: no numeric key versions returned for keyId=" + keyId.value()));
