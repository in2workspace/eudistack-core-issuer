...
public class GrantsServiceImpl implements GrantsService {
    ...
    public Mono<GrantsResult> createGrants(String processId, Mono<String> issuanceIdMono) {
        Mono<PreAuthorizedCodeResponse> preAuthMono = preAuthorizedCodeService.issuePreAuthorizedCode(processId, issuanceIdMono);
        Mono<String> issuerStateMono = generateCustomNonce();

        return Mono.zip(preAuthMono, issuerStateMono, issuanceIdMono)
                .flatMap(tuple -> {
                    PreAuthorizedCodeResponse preAuthResponse = tuple.getT1();
                    String issuerState = tuple.getT2();
                    String issuanceId = tuple.getT3();

                    return issuerStateCacheStore.add(issuerState, issuanceId)
                            .thenReturn(buildGrantsResult(processId, preAuthResponse, issuerState));
                });
    }

    private GrantsResult buildGrantsResult(String processId, PreAuthorizedCodeResponse preAuthResponse, String issuerState) {
