...
class IssuanceWorkflowImplTest {
    ...
    // --- AC-04 conformance reinforcement (EUD-71) ---
    //
    // Scope note: CredentialOfferService is mocked throughout this class, so the content
    // of the credential_offer object (grants, credential_offer_uri) is NOT verified here --
    // that's CredentialOfferServiceImplTest. What is the workflow's responsibility, and had
    // no explicit assertion in the 14 pre-existing tests, is that the persisted Issuance
    // entity references the same profile/format the issuance was triggered with -- that's
    // what these two tests close.
    //
    // The other 3 cases an initial draft of this task proposed (default grant type,
    // config id propagated to the offer, delivery combined with cnf_required) turned out to
    // be near-literal duplicates of executeShouldCompleteFullIssuanceFlowWithEmailDelivery,
    // directDeliveryShouldUseTokenJwtStatusFormatForDcSdJwtCredential y
    // directDeliveryShouldFailWhenCnfIsRequired -- no se repiten aqui.
