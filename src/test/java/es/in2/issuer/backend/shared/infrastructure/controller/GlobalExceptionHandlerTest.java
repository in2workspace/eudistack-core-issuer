...
// -------------------- handleRemoteSignatureException --------------------
    ...
    void handleRemoteSignatureException() {
        var ex = new RemoteSignatureException("remote server timeout");
        var type = GlobalErrorTypes.REMOTE_SIGNATURE_ERROR.getCode();
        var title = "Remote signature error";
        var st = HttpStatus.BAD_GATEWAY;
        var fallback = "An error occurred during remote signature.";
        ...
        StepVerifier.create(handler.handleRemoteSignatureException(ex, request))
                .assertNext(gem -> assertGem(gem, type, title, st, "remote server timeout"))
                .verifyComplete();
