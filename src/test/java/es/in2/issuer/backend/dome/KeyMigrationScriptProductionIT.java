...
@TestPropertySource(properties = {
        ...
        "issuer.dome.key-migration.legacy-key-id=prod-legacy-key",
        "issuer.dome.key-migration.kms-alias=alias/dome/signing"
})
@DisplayName("AC-02: KeyMigration production import — executeProduction transitions DB to PLAN_A_OK")
class KeyMigrationScriptProductionIT {

    private static final String LEGACY_KEY_ID = "prod-legacy-key";
    ...
    void stubMocks() {
        ...
        when(vaultExportPort.exportWrapped(any(LegacyKeyId.class), any()))
                .thenReturn(Mono.just(new EncryptedKeyEnvelope(new byte[]{4, 5, 6}, "RSAES_OAEP_SHA_256", "")));
    ...
    @Test
    @DisplayName("executeProduction_WhenKmsRespondsOk_TransitionsDbToPlanAOk")
    void executeProduction_WhenKmsRespondsOk_TransitionsDbToPlanAOk() {
        // Act
    ...
    @Test
    @DisplayName("executeProduction_WhenKmsRespondsOk_AuditEntryRecorded")
    void executeProduction_WhenKmsRespondsOk_AuditEntryRecorded() {
        // Act
        keyMigrationWorkflow.executeProduction(LEGACY_KEY_ID).block();
