...
public class SyncCredentialsCacheTtlWindowIT {
    ...
    static class CacheTimeTravelConfig {
        ...
        public Cache<IdempotencyCacheKey, SyncCredentialsResult> timeTravelCache(FakeTicker ticker) {
            return Caffeine.newBuilder()
                    .expireAfterWrite(Duration.ofMinutes(5)) // TTL: 300 seconds
                    .ticker(ticker)
                    .maximumSize(10_000)
                    .build();
        }
    ...
    @Test
    @DisplayName("NFR-S-144-02: Test TTL window (cached at 270s, expired at 330s)")
    void syncCredentialsCacheTtlWindow() {
        ...
        // 1. T=0s: Initial Request (Hits DB)
        executePost(requestBody)
                .expectStatus().isOk()
                .expectHeader().doesNotExist("Idempotent-Replay");

        // 2. TIME TRAVEL: Advance to T=270s (Still within 300s window)
        fakeTicker.advance(270, TimeUnit.SECONDS);

        // 3. T=270s: Request within valid window (Hits Cache)
        executePost(requestBody)
                .expectStatus().isOk()
                .expectHeader().valueEquals("Idempotent-Replay", "true");

        // 4. TIME TRAVEL: Advance another 60s -> Total T=330s (Past 300s window)
        fakeTicker.advance(60, TimeUnit.SECONDS);

        // 5. T=330s: Request outside valid window (Cache Expired, Hits DB)
        executePost(requestBody)
                .expectStatus().isOk()
                .expectHeader().doesNotExist("Idempotent-Replay");

        // VERIFY: DB hit exactly 2 times (T=0s and T=330s). The one at T=270s was cached.
        verify(credentialSyncPort, times(2)).findByHolderKey(anyString(), any());
    }

    // Helper method to keep test clean
    private WebTestClient.ResponseSpec executePost(String requestBody) {
