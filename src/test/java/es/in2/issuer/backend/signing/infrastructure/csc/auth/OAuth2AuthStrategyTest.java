...
class OAuth2AuthStrategyTest {
    ...
    private OAuth2AuthStrategy client;
    private RemoteSignatureDto cfg;

    @BeforeEach
    void setUp() {
        ObjectMapper objectMapper = new ObjectMapper();
        client = new OAuth2AuthStrategy(objectMapper, hashGeneratorService, httpUtils);
    ...
    void requestAccessToken_returnsToken_whenAccessTokenPresent() {
        ...
        StepVerifier.create(client.requestAccessToken(request("x"), "some-scope", true))
    ...
    void requestAccessToken_includesAuthorizationDetails_whenScopeIsCredential() {
        ...
        StepVerifier.create(client.requestAccessToken(sr, SIGNATURE_REMOTE_SCOPE_CREDENTIAL, true))
                .assertNext(token -> assertEquals("tok123", token))
                .verifyComplete();
    ...
    @Test
    void supportedProvider_returnsOAuth2() {
        assertEquals(CscAuthProvider.OAUTH_2, client.supportedProvider());
    }
}
